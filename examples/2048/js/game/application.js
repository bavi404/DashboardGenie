window.requestAnimationFrame(() => {
  ...
  initializeKeenClient();
  initializeGame();

  function initializeKeenClient() {
  ...
  function setupCharts() {
    window.chart_total_moves = new Keen.Dataviz()
      .el('#moves-container')
      .height(200)
      .title('Total Moves')
      .type('metric')
    ...
    window.chart_best_scores = new Keen.Dataviz()
      .el('#stats-container')
      .height(200)
      .title('High Score')
      .type('metric')
  ...
  function fetchAndRenderData() {
    ...
    .then((res) => chart_total_moves.data(res).render())
    .catch((err) => chart_total_moves.message(err.message));

    client.query('maximum', {
      event_collection: 'new_high_score2',
      target_property: 'best_score'
    })
    .then((res) => chart_best_scores.data(res).render())
    .catch((err) => chart_best_scores.message(err.message));
  }
